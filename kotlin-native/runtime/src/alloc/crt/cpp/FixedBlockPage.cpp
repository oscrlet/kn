...
void FixedBlockPage::ReusePages(uint32_t blockSize) noexcept {
    CustomAllocInfo("FixedBlockPage::ReusePages(%u)", blockSize);
    allocatedSizeTracker_.reset();
    blockSize_ = blockSize;
    nextFree_.first = 0;
    nextFree_.last = cellCount() / blockSize * blockSize;
    end_ = cellCount() / blockSize * blockSize;
    Reuse(this, FixedBlockPage::SIZE);
}

void FixedBlockPage::RecyclePages() noexcept {
    CustomAllocInfo("FixedBlockPage::RecyclePages()");
    Recycle(this, FixedBlockPage::SIZE);
}
