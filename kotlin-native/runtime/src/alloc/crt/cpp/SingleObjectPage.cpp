...
void SingleObjectPage::ReusePages(uint32_t blockSize) noexcept {
    CustomAllocError("SingleObjectPage can't ReusePages()");
    abort();
}

void SingleObjectPage::RecyclePages() noexcept {
    CustomAllocError("SingleObjectPage can't RecyclePages()");
    abort();
}
