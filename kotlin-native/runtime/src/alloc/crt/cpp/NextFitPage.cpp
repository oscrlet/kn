...
void NextFitPage::ReusePages(uint32_t blockSize) noexcept{
    CustomAllocInfo("NextFitPage::ReusePages(%u)", blockSize);
    allocatedSizeTracker_.reset();
    curBlock_ = cells_;
    cells_[0] = Cell(0); // Size 0 ensures any actual use would break
    cells_[1] = Cell(NextFitPage::cellCount() - 1);
    Reuse(this, NextFitPage::SIZE);
}

void NextFitPage::RecyclePages() noexcept{
    CustomAllocInfo("NextFitPage::RecyclePages()");
    Recycle(this, NextFitPage::SIZE);
}
